...
This document describes differences between the 5.005 release and the 5.6.3
release.

=head1 Summary of changes between 5.6.2 and 5.6.3

The 5.6.3 release is aimed at providing only a minimal update to perl 5.6.2,
applying security fixes and compiler and shell compatibility.

=head2 Security Issues

Hash randomization was backported from 5.8.1, but rehashing on hsplit not.
So hashes are always randomized, not only when being enhanced.
...
perl #111594 Socket heap-buffer-overflow on linux with abstract AF_UNIX paths.
